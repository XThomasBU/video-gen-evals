...
for MODEL in ["wan21", "runway_gen4", "hunyuan_360p", "opensora_256p", "cogvideox"]:
    ...
    # global_stats['body_pose_mean'] = np.load('SAVE/body_pose_mean.npy')
    # global_stats['body_pose_std'] = np.load('SAVE/body_pose_std.npy')
    ...
    if os.path.exists(f"SAVE_TEST/gen_dataset_window_{WINDOW_SIZE}_stride_{STRIDE}_valid_window_MODEL_{MODEL}.pt"):
        ...
        # save gen dataset for the model
    ...
    with torch.no_grad():
        for seqs, labels, vid_names in tqdm(gen_loader):
            ...
            # centroids_norm = {k: v.cpu() for k, v in centroids.items()}
    ...
    for i, cls in enumerate(ALL_CLASSES):
        ...
        real_embeds[i] = all_train_embeds[mask].numpy()  # [N_real, D]
    for vid, vals in video_results.items():
        ...
        spread = torch.norm(emb_stack - video_mean_emb, dim=1).mean().item()

        # --- Energy score (distance of video mean to all real embeds of this class) ---
        cls_idx = vals["cls"]
        real_embeds_cls = real_embeds[cls_idx]  # [N_real, D]
        video_mean_np = video_mean_emb.cpu().numpy().squeeze()
        dists_to_real = np.linalg.norm(real_embeds_cls - video_mean_np, axis=1)
        energy = dists_to_real.mean()

        # --- Percentile/Hausdorff Penalty (e.g., 95th percentile distance to real) ---
        percentile_penalty = np.percentile(dists_to_real, 95)

        # --- Hausdorff (max-min) distance (optional, more sensitive to outliers) ---
        hausdorff_score = dists_to_real.max()  # Or use scipy.spatial.directed_hausdorff if you want two-way

        # --- Composite penalized score (tune weights as needed) ---
        beta = 1.0
        gamma = 1.0
        delta = 1.0
        penalized_score = (
    ...
    # Sort by mean intra
    video_scores.sort(key=lambda x: x[2])  # Sort by mean_intra ascending
    ...
    for rank, (vid, cls_name, intra, cons, spread, energy, percentile_penalty, hausdorff_score, penalized_score, max_intra) in enumerate(video_scores, 1):
        print(f"{rank}. {vid} | Class: {cls_name} | Intra: {intra:.4f} | Weighted Consistency: {cons:.4f} | Spread: {spread:.4f} | Energy: {energy:.4f} | 95th Perc Penalty: {percentile_penalty:.4f} | Hausdorff: {hausdorff_score:.4f} | Penalized Score: {penalized_score:.4f} | Max Intra: {max_intra:.4f}")
    ...
    for cls_name, videos in class_video_ranking.items():
        ...
        for rank, (vid, intra, cons, spread, energy, percentile_penalty, hausdorff_score, penalized_score, max_intra) in enumerate(top3, 1):
            print(f"  {rank}. {vid} | Consistency: {cons:.4f} | Intra: {intra:.4f} | Spread: {spread:.4f} | Energy: {energy:.4f} | Percentile Penalty: {percentile_penalty:.4f} | Hausdorff Score: {hausdorff_score:.4f} | Penalized Score: {penalized_score:.4f} | Max Intra: {max_intra:.4f}")
    ...
    SAVE_DIR = f"/projectnb/ivc-ml/xthomas/RESEARCH/video_evals/video-gen-evals/src/train_FINAL/SAVE_TEST/models/{MODEL}_NO_ENT"
    for cls_name, videos in class_video_ranking.items():
        # Sort by consistency instead of penalized score
        videos.sort(key=lambda x: x[1])  # x[2] is consistency
    ...
    # Prepare to collect per-class scores for summary table
    class_summary = []

    for cls_name, videos in class_video_ranking.items():
        # Each entry: (vid, intra, cons, spread, penalized_score)
        n = len(videos)
    ...
    # # Optionally, save to CSV for later model comparison
    df_summary.to_csv(f"SAVE_TEST/jsons/gen_{GEN_ROOT.split('/')[-1]}_per_class_avg_scores_window_{WINDOW_SIZE}_stride_{STRIDE}_valid_window.csv", index=False)
    print(f" Saved per-class summary as gen_{GEN_ROOT.split('/')[-1]}_per_class_avg_scores_window_{WINDOW_SIZE}_stride_{STRIDE}_valid_window.csv")

    # real_embeds_per_class = {}
    # for i, cls in enumerate(ALL_CLASSES):
    #     # Mask for this class
    #     mask = (all_train_labels == i)
    #     # Get all embeddings for this class
    #     real_embeds_per_class[cls] = all_train_embeds[mask]

    # from scipy.spatial.distance import pdist

    # print("\n📊 Per-Class Comparison to Real Distribution:\n")
    # real_vs_gen_summary = []

    # for cls_name in class_video_ranking:
    #     real_embeds = real_embeds_per_class[cls_name]  # [N_real, D]
    #     real_centroid = real_embeds.mean(dim=0)
    #     gen_vids = [v[0] for v in class_video_ranking[cls_name]]
    #     gen_embeds = torch.stack([torch.stack(video_embeds[vid]).mean(dim=0) for vid in gen_vids])
    #     real_to_centroid = torch.norm(real_embeds - real_centroid, dim=1).numpy()
    #     gen_to_real_centroid = torch.norm(gen_embeds - real_centroid, dim=1).numpy()
    #     real_spread = pdist(real_embeds.numpy()).mean() if len(real_embeds) > 1 else 0.0
    #     gen_spread = pdist(gen_embeds.numpy()).mean() if len(gen_embeds) > 1 else 0.0
    #     dists = torch.cdist(gen_embeds, real_embeds)
    #     min_real_nn = dists.min(dim=1)[0].mean().item()
    #     real_vs_gen_summary.append({
    #         "class": cls_name,
    #         "num_real": len(real_embeds),
    #         "num_gen": len(gen_embeds),
    #         "real_mean_centroid_dist": real_to_centroid.mean(),
    #         "gen_mean_centroid_dist": gen_to_real_centroid.mean(),
    #         "real_spread": real_spread,
    #         "gen_spread": gen_spread,
    #         "gen_mean_to_nearest_real": min_real_nn,
    #     })
    #     print(
    #         f"Class: {cls_name}\n"
    #         f"  Real: mean dist to centroid = {real_to_centroid.mean():.4f} (N={len(real_embeds)}) | spread = {real_spread:.4f}\n"
    #         f"  Gen : mean dist to real centroid = {gen_to_real_centroid.mean():.4f} (N={len(gen_embeds)}) | spread = {gen_spread:.4f}\n"
    #         f"  Gen: mean distance to nearest real = {min_real_nn:.4f}\n"
    #     )

    # df_real_vs_gen = pd.DataFrame(real_vs_gen_summary)
    # # df_real_vs_gen.to_csv(
    # #     f"SAVE_NEW2/jsons/gen_{GEN_ROOT.split('/')[-1]}_real_vs_gen_comparison_window_{WINDOW_SIZE}_stride_{STRIDE}_valid_window.csv",
    # #     index=False,
    # # )
    # print(f" Saved real-vs-gen comparison as gen_{GEN_ROOT.split('/')[-1]}_real_vs_gen_comparison_window_{WINDOW_SIZE}_stride_{STRIDE}_valid_window.csv")