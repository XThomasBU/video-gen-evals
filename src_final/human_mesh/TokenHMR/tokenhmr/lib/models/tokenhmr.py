...
sys.path.append(
    os.path.join(__file__.replace(os.path.basename(__file__), ""), "..", "..", "..")
)
...
class TokenHMR(pl.LightningModule):
    ...
    def __init__(
        ...
        if not is_train_state:
            ...
            self.backbone, self.smpl_head = create_backbone(
    ...
    def forward_step(self, batch: Dict, train: bool = False) -> Dict:
        ...
        smpl_output = self.smpl(
            **{k: v for k, v in pred_smpl_params.items()}, pose2rot=False
        )
    ...
    def compute_loss(
        ...
        if self.cfg.MODEL.LOOSE_SUP and train:
            ...
            valid_mask2D = kp2D_err > kp2D_err_valid_thresh[None].repeat(
                batch_size, 1
            ).to(kp2D_err.device)
            ...
            loss_keypoints_2d = self.keypoint_2d_loss(
                ...
                gt_keypoints_2d,
                weak_mask,
                self.cfg.MODEL.LOOSE_WEIGHT,
            )

            valid_3D_mask = (
                torch.Tensor(
                    [name in ["H36M-TRAIN-WMASK", "BEDLAM"] for name in dataset_names]
                )
                .float()
                .to(gt_keypoints_3d.device)
            )
            gt_keypoints_3d[:, :, -1] = gt_keypoints_3d[:, :, -1] * (
                (valid_3D_mask.unsqueeze(-1) + gt_keypoints_2d[:, :, -1]) > 0.5
            )
            ...
            for k, pred in pred_smpl_params.items():
                ...
                if k in ["betas"]:
                    ...
                    valid_mask3D = angle_error > angle_valid_thresh[k][None].repeat(
                        batch_size, 1
                    ).to(angle_error.device)
                    valid_mask3D = (
                        valid_mask3D * has_gt.unsqueeze(1) + valid_3D_mask.unsqueeze(1)
                    ).bool()
    ...
    def tensorboard_logging(
        ...
        pred_keypoints_2d = (
            output["pred_keypoints_2d"].detach().reshape(batch_size, -1, 2)
        )
    ...
    def training_step(self, joint_batch: Dict, batch_idx: int) -> Dict:
        ...
        if self.cfg.LOSS_WEIGHTS.ADVERSARIAL > 0:
            ...
            print(
                "dis gan loss:"
                + "{:.2f}".format(self.cfg.LOSS_WEIGHTS.ADVERSARIAL * loss_adv)
            )
